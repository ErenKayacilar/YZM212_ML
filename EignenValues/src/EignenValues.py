...
def find_determinant(matrix, excluded=1):
    ...
    # 2x2 matris için temel durum (base case)
    if dimensions == [2, 2]:
        return excluded * ((matrix[0][0] * matrix[1][1]) - (matrix[0][1] * matrix[1][0]))

    # Büyük matrisler için Laplace açılımı (ilk satır boyunca)
    new_matrices = []
    excluded_vals = []
    exclude_row = 0
